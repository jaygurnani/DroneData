...
# Read the data
data <- read.csv(file="circlezigzag.csv", header=TRUE, sep=",")
data2 <- read.csv(file="45mscircle.csv", header=TRUE, sep=",")
...
data2.toPlot<- data2[data2$Battery > 22,]
...
data2.toPlot$TimeDiff <- data.toPlot$Date - data.toPlot$Date[1]
...
data.toPlotFinala <- data[data.toPlot$TimeDiffValue < 6]

# Multiple current and voltage to get Power
data.toPlot$Power <- as.double(-(data.toPlot$Current/1000) * data.toPlot$Voltage/1000)
data2.toPlot$Power <- as.double(-(data.toPlot$Current/1000) * data.toPlot$Voltage/1000)
...
data2.toPlot$Velocity <- sqrt(data.toPlot$VelocityX^2 + data.toPlot$VelocityY^2 + data.toPlot$VelocityZ^2)
#data.toPlotFinal <- data.toPlot[data.toPlot$TimeDiffValue > 9 & data.toPlot$TimeDiffValue < 17, ]
...
#powerTest <- data.toPlot$Power[1:141]
#ppi <- 200
#png(file=paste("circlezigzag", ".png", sep=""), height=7*ppi, width=9*ppi, res=ppi)

#write.csv(data.toPlotFinal, file="FirstRev.csv")
plot(y=data.toPlot$DroneHeading, x=data.toPlot$TimeDiffValue,type="l",col="black", lwd="1", ylab="Velocity (m/s)", xlab="Power (Watts)")
